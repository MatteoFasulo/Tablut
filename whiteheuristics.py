...
def is_white_gonna_be_eaten(board,alpha0):
    ...
    for white in board.whites:
        if white[0] > 0 and board.pieces[white[0]-1][white[1]] == 1 and can_this_tile_be_reached_by_a_black_pawn(board, white[0]+1, white[1]):
            fitness += alpha0

        if white[0] < len(board.pieces)-1 and board.pieces[white[0]+1][white[1]] == 1 and can_this_tile_be_reached_by_a_black_pawn(board, white[0]-1, white[1]):
            fitness += alpha0

        if white[1] > 0 and board.pieces[white[0]][white[1]-1] == 1 and can_this_tile_be_reached_by_a_black_pawn(board,  white[0], white[1]+1):
            fitness += alpha0
...
def external_pawn_more_fitness(board, gamma0):
    ...
    fitness = 0
    for white in board.whites:
        fitness += gamma0 * ((white[0] - board.king[0])**2 + (white[1] - board.king[1])**2)**0.5
...
def white_fitness(board, alpha0, beta0, gamma0, theta0, epsilon0):
    ...
    #Number of pawns
    fitness += len(board.whites)*epsilon0
    return fitness
